...
public class SmartShootVelocityCommand extends Command {
    ...
    public void execute() {
		...
		if (Math.abs(setpoint.velocity - m_shooterSubsystem.getLeaderMotorVelocitySignal()) < Constants.FeederConstants.SPINUP_THRESHOLD) {
			// run kicker if shooter is at the right speed
			m_feederSubsystem.runFeederVelocity(Constants.FeederVelocitySetpoint.Feed.velocity);
    	}
